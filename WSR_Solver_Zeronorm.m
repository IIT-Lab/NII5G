function [User,sumrate, wgt_sumrate,Per_BS_Bkhaul, Cells] = ...
    WSR_Solver_Zeronorm(Num_Cell,K,Cells,User,P_max_total_dBm,Chn,noise_dB,snr_gap_dB,...
...
while (abs(current_rate - previous_rate)/current_rate) > 1e-3 ||  iteration <= 30
    ...
    %if Power_Mode == 3
        [User, max_diff] = CVX_QCQP_TxBF_Zero(User,Cells,N_k,Num_Cell,Power_Mode,Chn,L_Macro,L_Pico,...
